...
| :--- | :--- | :---: | :---: |
| **อุปกรณ์ที่ 1** | 06:DB:F1:BD:D4:DF | 1 | 192.168.4.2 |
| **อุปกรณ์ที่ 2** | C2:62:4D:96:AB:0E | 2 | 192.168.4.3 |
...
**ตอบ:** `192.168.4.1` เป็นค่า Default Gateway IP ที่ถูกกำหนดมาเป็นมาตรฐานในสถาปัตยกรรมของ ESP-IDF สำหรับการทำงานในโหมด SoftAP โดยบริการ DHCP Server บน ESP32 จะเริ่มแจกจ่าย IP ให้กับ Client (อุปกรณ์ลูกข่าย) เริ่มต้นที่หมายเลข **`192.168.4.2`** และรันต่อไปตามลำดับเครื่องที่เข้ามาเชื่อมต่อ
...
**ตอบ:** ข้อมูล `mac` ที่ดักจับได้จาก Event นี้ สามารถนำไปตรวจสอบเทียบกับฐานข้อมูลหรือ Array ที่เรากำหนดไว้ในโค้ด (เช่น Whitelist หรือ Blacklist) หากพบว่ามีอุปกรณ์แปลกปลอมที่ MAC Address ไม่ตรงกับรายชื่อที่อนุญาต โค้ดของเราสามารถสั่งเรียกใช้ฟังก์ชัน `esp_wifi_deauth_sta()` เพื่อเตะ (Deauthenticate) อุปกรณ์นั้นออกจากการเชื่อมต่อในระดับ Link Layer ได้ทันที

**3. หากมี Client พยายามเชื่อมต่อเป็นเครื่องที่ 5 (เกินค่า `max_connection = 4`) จะเกิดเหตุการณ์ใดขึ้นในระดับสัญญาณวิทยุ?**
**ตอบ:** ในระดับสัญญาณวิทยุ (MAC Layer) เมื่อ AP มี Client เชื่อมต่อครบ 4 เครื่องตามที่กำหนดใน `max_connection` แล้ว หากมีเครื่องที่ 5 ส่งคำขอเชื่อมต่อ (Association Request) เข้ามา ESP32 จะตอบกลับด้วยเฟรม **Association Response พร้อมกับระบุสถานะปฏิเสธ (Reject/Denied)** ทันที ทำให้สมาร์ทโฟนหรืออุปกรณ์เครื่องที่ 5 ไม่สามารถเข้าร่วมเครือข่ายได้
