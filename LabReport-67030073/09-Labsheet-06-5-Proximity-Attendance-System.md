...
### 5.1 ตารางบันทึกการเช็คชื่อผ่าน RF Proximity

| ลำดับที่ | ชื่อสมาร์ทโฟน / MAC Address | ระดับ RSSI (dBm) | ระยะทางประเมิน (Near/Far) | ผลการลงชื่อ (Passed/Rejected) |
| :---: | :--- | :---: | :---: | :---: |
| **1** | Phone A / `46:19:D0:9F:EB:37` | -45 | Near | Passed |
| **2** | Phone B / `AA:9C:9E:03:03:99` | -78 | Far | Rejected |

---

## 6. คำถามท้ายการทดลอง (Post-Lab Questions)

**1. การใช้ RF Signal Proximity (RSSI) ร่วมกับ HTTP Web Server บน ESP32 แก้ปัญหาการฝากเช็คชื่อแทนกันในห้องเรียนได้อย่างไร?**  
**ตอบ:** ระบบนี้บังคับให้สมาร์ทโฟนของนักศึกษาต้องเชื่อมต่อกับ Wi-Fi (SoftAP) ของ ESP32 และต้องอยู่ใกล้กับบอร์ดในรัศมีที่กำหนดเท่านั้น (อ้างอิงจากความแรงสัญญาณ RSSI) หากมีนักศึกษาส่ง URL หรือ IP Address ไปให้เพื่อนที่ไม่ได้อยู่ในห้องเรียน ตัว ESP32 จะตรวจสอบพบว่าไม่มี MAC Address ของเพื่อนคนนั้นในรัศมีสัญญาณที่กำหนด (สัญญาณอ่อนเกินไป หรือไม่ได้เชื่อมต่อ AP โดยตรง) ระบบจึงสามารถปฏิเสธ (Reject) การลงชื่อของคนที่ไม่ได้อยู่ในห้องจริงได้อย่างแม่นยำ

**2. เหตุใดระดับเกณฑ์ RSSI ที่ `-55 dBm` จึงเหมาะสมสำหรับการระบุตำแหน่งอุปกรณ์ให้อยู่ภายในรัศมีโต๊ะปฏิบัติการ?**  
**ตอบ:** ค่าความแรงสัญญาณที่ `-55 dBm` เป็นระดับสัญญาณที่ค่อนข้างแรง ซึ่งในทางปฏิบัติจะเทียบเท่ากับระยะห่างประมาณ 1-2 เมตรโดยไม่มีสิ่งกีดขวางหนาแน่น ซึ่งครอบคลุมบริเวณโต๊ะปฏิบัติการพอดี 
* หากตั้งเกณฑ์ต่ำไป (เช่น `-80 dBm`) รัศมีจะกว้างไปถึงนอกห้องเรียน ทำให้คนนอกห้องสามารถกดยืนยันตัวตนได้ 
* หากตั้งเกณฑ์สูงไป (เช่น `-30 dBm`) รัศมีจะแคบเกินไปจนนักศึกษาอาจจะต้องนำโทรศัพท์มาแตะกับตัวบอร์ด ESP32 ถึงจะสามารถเช็คชื่อผ่านได้
...
**ตอบ:** จะต้องเพิ่มและปรับเปลี่ยนโครงสร้างการทำงาน 3 ส่วนหลัก ดังนี้:
1. **เปลี่ยนโหมดการทำงานของ Wi-Fi:** จากเดิมที่เป็น `WIFI_MODE_AP` (ปล่อยสัญญาณอย่างเดียว) ต้องเปลี่ยนเป็นโหมด `WIFI_MODE_APSTA` เพื่อให้ ESP32 สามารถปล่อยสัญญาณให้สมาร์ทโฟนเชื่อมต่อ และในขณะเดียวกันก็เกาะเครือข่าย Wi-Fi ของมหาวิทยาลัยเพื่อออกอินเทอร์เน็ตได้
2. **เพิ่ม HTTP/HTTPS Client:** นำไลบรารี `esp_http_client` มาใช้งาน เพื่อให้ ESP32 สามารถส่ง Request ข้อมูล (POST/GET) ออกไปยังเซิร์ฟเวอร์ภายนอกได้
3. **เพิ่มการจัดการข้อมูล JSON:** ใช้ไลบรารี `cJSON` ในการแพ็กเกจข้อมูล MAC Address, เวลา และสถานะการเช็คชื่อ ให้อยู่ในรูปแบบโครงสร้าง JSON ก่อนส่งขึ้นไปประมวลผลเก็บบน Database หรือ Webhook ของระบบ Cloud
