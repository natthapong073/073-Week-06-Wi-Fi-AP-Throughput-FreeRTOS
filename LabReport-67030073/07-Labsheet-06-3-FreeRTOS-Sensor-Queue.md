...
| ชื่อ FreeRTOS Task | ขนาด Stack ที่กำหนดใน `xTaskCreate` (Bytes) | ค่า High Water Mark ที่อ่านได้ (Bytes) | สถานะความปลอดภัยสแตก |
| :--- | :---: | :---: | :--- |
| **SensorCollectorTask** | 3072 | ~1540 bytes | ปลอดภัย (มีพื้นที่เหลือเพียงพอ) |
| **SensorCollectorTask** | 1024 | 0 bytes | **อันตราย (เกิด Stack Overflow และบอร์ดรีบูต)** |
| **NetworkCommTask** | 4096 | ~2120 bytes | ปลอดภัย (มีพื้นที่เหลือเพียงพอ) |
...
**ตอบ:** การใช้ FreeRTOS Queue มีความปลอดภัยกว่าตัวแปรแบบ Global เนื่องจากระบบ Queue มีกลไกการจัดการการเข้าถึงข้อมูลร่วมกันแบบ Mutual Exclusion ป้องกันไม่ให้เกิดปัญหา Race Condition หรือข้อมูลถูกเขียนทับพร้อมกันขณะที่อีก Task กำลังอ่าน นอกจากนี้ยังมีกลไก Blocking state ที่ช่วยให้ Task รอข้อมูลได้อย่างมีประสิทธิภาพโดยไม่ต้องวนลูปเช็ค (Busy-wait)

**2. ค่า Stack High Water Mark มีประโยชน์อย่างไรในการตรวจวินิจฉัยปัญหาบั๊กในระบบเรียลไทม์ (RTOS)?**
**ตอบ:** ค่า Stack High Water Mark ใช้บอกถึง "พื้นที่สแตกเหลือต่ำที่สุด" (Minimum remaining stack space) ตลอดระยะเวลาที่ Task นั้นรันอยู่ มีประโยชน์อย่างมากในการทำ Stack Profiling เพื่อช่วยให้ Developer สามารถปรับลดหรือเพิ่มขนาด Stack ให้เหมาะสม ไม่เผื่อไว้ใหญ่เกินไปจนเปลืองแรม และช่วยเตือนภัยล่วงหน้าก่อนจะเกิดปัญหา Stack Overflow ที่ทำให้ระบบล่มหรือรีบูตกระทันหัน
...
**ตอบ:** จะทำให้ข้อมูลถูกผลิตเร็วกว่าอัตราการบริโภค ส่งผลให้ **Queue เต็ม (Queue Full)** อย่างรวดเร็ว ในระดับโค้ด เมื่อฟังก์ชัน `xQueueSend()` พยายามส่งข้อมูลขณะคิวเต็มและหมดเวลา Timeout ฟังก์ชันจะคืนค่าเป็น `errQUEUE_FULL` ทำให้ระบบไม่สามารถนำข้อมูลใหม่ใส่เข้าไปได้ (เกิด Data Loss สำหรับข้อมูลใหม่) จนกว่า Network Task จะดึงข้อมูลเก่าออกจากคิวไปประมวลผลสำเร็จเพื่อให้มีพื้นที่ว่างครับ
